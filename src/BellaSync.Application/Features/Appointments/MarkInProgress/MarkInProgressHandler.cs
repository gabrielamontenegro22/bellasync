...
public sealed class MarkInProgressHandler
    : ICommandHandler<MarkInProgressCommand, AppointmentResponse>
{
    private readonly IApplicationDbContext _db;
    private readonly IClock _clock;

    public MarkInProgressHandler(IApplicationDbContext db, IClock clock)
    {
        _db = db;
        _clock = clock;
    }
    ...
    public async Task<Result<AppointmentResponse>> HandleAsync(
        MarkInProgressCommand command, CancellationToken ct)
    {
        ...
        try { appointment.MarkInProgress(_clock.UtcNow); }
